...
class RunCoverageTest {
  ...
  fun testRunCoverage_testFileExempted_exemptedFromCoverageAnalysis() {
    ...
    val exemptionsReferenceNote =
      ">Refer [test_file_exemptions.textproto]" +
    ...
    val testFileExemption =
      TestFileExemptions.TestFileExemption
        .newBuilder()
        .apply {
          this.exemptedFilePath = exemptedFile
          this.testFileNotRequired = true
        }.build()
    val testFileExemptions =
      TestFileExemptions
        .newBuilder()
        .apply {
  ...
  fun testRunCoverage_withNonKotlinFileInput_analyzeOnlyKotlinFiles() {
    ...
    val unescapedHtmlReport =
      readHtmlReport(kotlinFilePath)
        .replace("&quot;", "\"")
        .replace("&amp;", "&")
        .replace("&lt;", "<")
        .replace("&gt;", ">")
  ...
  fun testRunCoverage_withMissingTestDeclarations_generatesFailureReport() {
    ...
    val failureMessage =
      "Missing test declaration(s) for existing test file(s): " +
        "${listOf("coverage/test/java/com/example/AddNumsTest.kt")}."

    val expectedFailureReport =
      "| ${getFilenameAsDetailsSummary(filePathList.get(0))} " +
        "| $failureMessage |"
  ...
  fun testRunCoverage_withMultipleTestsHittingSameLine_calculatesCoverageReportCorrectly() {
    val filePathList = listOf("app/main/java/com/example/AddNums.kt")
    val protoOutputPath =
      "${tempFolder.root}/coverage_reports/" +
  ...
  fun testRunCoverage_withMultipleFilesHtmlFormat_generatesCoverageReport() {
    ...
    val unescapedHtmlReport1 =
      readHtmlReport(filePathList.get(0))
        .replace("&quot;", "\"")
        .replace("&amp;", "&")
        .replace("&lt;", "<")
        .replace("&gt;", ">")
    ...
    val unescapedHtmlReport2 =
      readHtmlReport(filePathList.get(1))
        .replace("&quot;", "\"")
        .replace("&amp;", "&")
        .replace("&lt;", "<")
        .replace("&gt;", ">")
  ...
  private fun createTestFileExemptionsProtoFile(testFileExemptions: TestFileExemptions): String =
    tempFolder
      .newFile("test_file_exemptions.pb")
      .also {
        it.outputStream().use(testFileExemptions::writeTo)
      }.path

  private fun getExpectedClassName(filePath: String): String = filePath.substringAfterLast("/").removeSuffix(".kt")
  ...
  private val oppiaCoverageWikiPageLinkNote =
    buildString {
      val wikiPageReferenceNote =
        ">To learn more, visit the [Oppia Android Code Coverage]" +
  ...
  private fun loadCoverageReportProto(coverageReportProto: String): CoverageReport =
    File("$coverageReportProto").inputStream().use { stream ->
      CoverageReport
        .newBuilder()
        .also { builder ->
