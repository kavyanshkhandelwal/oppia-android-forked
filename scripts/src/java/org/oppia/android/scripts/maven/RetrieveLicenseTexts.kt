...
class RetrieveLicenseTexts(
  ...
  private fun parseTextProto(
    ...
    return FileInputStream(protoBinaryFile)
      .use {
        builder.mergeFrom(it)
      }.build() as MavenDependencyList
  }

  private fun retrieveAllLicensesSet(mavenDependencyList: List<MavenDependency>): Set<CopyrightLicense> {
  ...
  private fun retrieveDependencyList(mavenDependencyList: List<MavenDependency>): List<Dependency> =
    mavenDependencyList.map { mavenDependency ->
      val copyrightLicenseList =
  ...
  private fun fetchViewableLicenseText(licenseLink: String): String {
    val licenseText =
      checkNotNull(fetchLicenseText(licenseLink)) {
  ...
  private fun retrieveArtifactsNamesList(dependencyList: List<Dependency>): List<String> = dependencyList.map { omitVersion(it.name) }

  private fun retrieveArtifactsVersionsList(dependencyList: List<Dependency>): List<String> = dependencyList.map { it.version }
  ...
  private fun omitVersion(artifactName: String): String = artifactName.substring(0, artifactName.lastIndexOf(':'))
