...
class ProfileManagementControllerTest {
  ...
  private companion object {
    private val PROFILES_LIST =
      listOf<Profile>(
        Profile
          .newBuilder()
          .setName("James")
          .setPin("123")
          .setAllowDownloadAccess(true)
          .build(),
        Profile
          .newBuilder()
          .setName("Sean")
          .setPin("234")
          .setAllowDownloadAccess(false)
          .build(),
        Profile
          .newBuilder()
          .setName("Ben")
  ...
  fun testGetProfiles_addManyProfiles_checkAllProfilesAreAdded() {
    ...
    val profiles =
      monitorFactory.waitForNextSuccessfulResult(dataProvider).sortedBy {
        it.id.internalId
      }
  ...
  fun testDeleteProfile_addProfiles_deleteProfiles_addProfile_checkIdIsNotReused() {
    ...
    val profiles =
      monitorFactory.waitForNextSuccessfulResult(profilesProvider).sortedBy {
        it.id.internalId
      }
  ...
  fun testFetchLastSelectedClassroomId_updateClassroomIds_checkUpdateIsSuccessfulPerProfile() {
    ...
    val profile0SelectedClassroomId =
      monitorFactory.waitForNextSuccessfulResult(
        profileManagementController.retrieveLastSelectedClassroomId(PROFILE_ID_0),
      )
  ...
  fun testUpdateExistingAdminProfile_updateProfileTypeToSupervisor_checkProfileTypeSupervisor() {
    ...
    val updateProvider =
      profileManagementController.updateProfileType(
        PROFILE_ID_0,
  ...
  private fun retrieveCurrentSessionId() = monitorFactory.waitForNextSuccessfulResult(loggingIdentifierController.getSessionId())
  ...
  private fun executeInPreviousAppInstance(block: (TestApplicationComponent) -> Unit) {
    ...
    block(
      DaggerProfileManagementControllerTest_TestApplicationComponent
        .builder()
