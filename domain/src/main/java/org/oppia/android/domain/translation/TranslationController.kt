...
class TranslationController
  ...
  constructor(
    ...
    fun getSystemLanguageLocale(): DataProvider<OppiaLocale.DisplayLocale> =
      getSystemLanguage().transformAsync(SYSTEM_LANGUAGE_LOCALE_DATA_PROVIDER_ID) { language ->
    ...
    fun getAppLanguage(profileId: ProfileId): DataProvider<OppiaLanguage> =
      getAppLanguageLocale(profileId).transform(APP_LANGUAGE_DATA_PROVIDER_ID) { locale ->
    ...
    fun getSupportedAppLanguages(): DataProvider<List<OppiaLanguage>> =
      dataProviders.createInMemoryDataProvider(RETRIEVED_CONTENT_LANGUAGE_DATA_PROVIDER_ID) {
        languageConfigRetriever
    ...
    fun getAppLanguageLocale(profileId: ProfileId): DataProvider<OppiaLocale.DisplayLocale> {
      ...
      return getSystemLanguage().combineWithAsync(
        getAppLanguageSelection(profileId),
        providerId,
     ...
     */
    fun getAppLanguageSelection(profileId: ProfileId): DataProvider<AppLanguageSelection> = retrieveAppLanguageContentCacheStore(profileId)
     ...
     */
    fun getWrittenTranslationContentLocale(profileId: ProfileId): DataProvider<OppiaLocale.ContentLocale> {
      val resolvedLanguageProvider =
        getWrittenTranslationContentLanguageSelection(profileId).combineWith(
          getAppLanguageSelection(profileId),
          WRITTEN_TRANSLATION_CONTENT_LANG_RES_DATA_PROVIDER_ID,
      ...
      return getSystemLanguage().combineWithAsync(
        ...
        WRITTEN_TRANSLATION_CONTENT_LOCALE_DATA_PROVIDER_ID,
     ...
     */
    fun getWrittenTranslationContentLanguageSelection(profileId: ProfileId): DataProvider<WrittenTranslationLanguageSelection> =
     ...
     */
    fun getAudioTranslationContentLocale(profileId: ProfileId): DataProvider<OppiaLocale.ContentLocale> {
      val resolvedLanguageProvider =
        getAudioTranslationContentLanguageSelection(profileId).combineWith(
          getAppLanguageSelection(profileId),
          AUDIO_TRANSLATION_CONTENT_LANG_RES_DATA_PROVIDER_ID,
      ...
      return getSystemLanguage().combineWithAsync(
        resolvedLanguageProvider,
        AUDIO_TRANSLATION_CONTENT_LOCALE_DATA_PROVIDER_ID,
      ) { systemLanguage, resolutionStatus ->
        val language = resolutionStatus.resolveToLanguage(systemLanguage)
     ...
     * result in no change to the underlying configured selection.
     */
    fun getAudioTranslationContentLanguageSelection(profileId: ProfileId): DataProvider<AudioTranslationLanguageSelection> =
    ...
    ): String = context.translationsMap[html.contentId]?.extractHtml() ?: html.html
     ...
     */
    fun extractString(
      unicode: SubtitledUnicode,
      context: WrittenTranslationContext,
    ): String = context.translationsMap[unicode.contentId]?.extractHtml() ?: unicode.unicodeStr
    ...
    fun extractStringList(
      ...
      context.translationsMap[translatableSetOfNormalizedString.contentId]?.extractHtmlList()
    ...
    fun computeWrittenTranslationContext(
      ...
      WrittenTranslationContext
        ...
        .apply {
          ...
          val contentMapping =
            writtenTranslationsMap.mapValuesNotNull { (_, mapping) ->
    ...
    private fun computeAppLanguage(languageSelection: AppLanguageSelection): LanguageResolutionStatus =
      when (languageSelection.selectionTypeCase) {
    ...
    private fun computeWrittenTranslationContentLanguage(
      ...
      when (contentLanguageSelection.selectionTypeCase) {
    ...
    private fun computeAudioTranslationContentLanguage(
      ...
      when (audioLanguageSelection.selectionTypeCase) {
    ...
    private fun retrieveAppLanguageContentCacheStore(profileId: ProfileId): PersistentCacheStore<AppLanguageSelection> =
      retrieveContentCacheStore(
        profileId,
    ...
    private fun getSystemLanguage(): DataProvider<OppiaLanguage> = localeController.retrieveSystemLanguage()
    ...
    private companion object {
      private fun Translation.extractHtml(): String? =
        takeIf {
          it.dataFormatCase == Translation.DataFormatCase.HTML
        }?.html

      private fun Translation.extractHtmlList(): List<String>? =
        takeIf {
      ...
      private fun LanguageResolutionStatus.resolveToLanguage(systemLanguage: OppiaLanguage): OppiaLanguage =
        when (this) {
