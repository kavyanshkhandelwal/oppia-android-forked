...
class TopicController
  ...
  constructor(
    ...
    fun getTopic(
      profileId: ProfileId,
      topicId: String,
    ): DataProvider<EphemeralTopic> = getTopics(profileId, listOf(topicId)).transform(GET_TOPIC_PROVIDER_ID) { it.single() }
    ...
    fun getTopics(
      ...
      val topicsCombinedProvider =
        topicsDataProvider.combineWith(
      ...
      return topicsCombinedProvider.combineWith(
        ...
        GET_LOCALIZABLE_TOPICS_PROVIDER_ID,
      ) { topics, locale ->
    ...
    fun getStory(
      ...
      val storyCombinedProvider =
        storyDataProvider.combineWith(
      ...
      return storyCombinedProvider.combineWith(
        translationLocaleProvider,
        GET_LOCALIZABLE_STORY_PROVIDER_ID,
    ...
    fun retrieveChapter(
      ...
      val chapterCombinedProvider =
        dataProviders
          .createInMemoryDataProviderAsync(
      ...
      return chapterCombinedProvider.combineWith(
        translationLocaleProvider,
        GET_LOCALIZABLE_CHAPTER_PROVIDER_ID,
            ...
            .apply {
              ...
              writtenTranslationContext =
                translationController.computeWrittenTranslationContext(
                  conceptCard.writtenTranslationMap,
                  contentLocale,
                )
            }.build()
        }
        ...
        .getWrittenTranslationContentLocale(
          ...
          EphemeralRevisionCard
            ...
            .apply {
              ...
              writtenTranslationContext =
                translationController.computeWrittenTranslationContext(
                  revisionCard.writtenTranslationsMap,
                  contentLocale,
                )
            }.build()
        }
    ...
    fun retrieveQuestionsForSkillIds(skillIdsList: List<String>): DataProvider<List<Question>> =
      dataProviders.createInMemoryDataProvider(RETRIEVED_QUESTIONS_FOR_SKILLS_ID_PROVIDER_ID) {
    ...
    private fun createOngoingTopicListFromProgress(
      ...
      val inProgressTopics =
        topicProgressList.mapNotNull { topicProgress ->
    ...
    private fun checkIfTopicIsOngoing(
      topic: Topic,
    ...
    private fun StoryProgress.getChapterPlayState(explorationId: String): ChapterPlayState =
      chapterProgressMap[explorationId]?.chapterPlayState ?: ChapterPlayState.NOT_STARTED
      ...
      storyProgressList.forEach { storyProgress ->
        ...
        if (storyProgress.chapterProgressMap.containsKey(lastChapterSummary.explorationId) &&
          ...
          val storyTranslationContext =
            translationController.computeWrittenTranslationContext(
              storySummary.writtenTranslationsMap,
              contentLocale,
            )
          val topicTranslationContext =
            translationController.computeWrittenTranslationContext(
              topic.writtenTranslationsMap,
              contentLocale,
            )
          val completedStoryBuilder =
            CompletedStory
              .newBuilder()
    ...
    private fun combineTopicAndTopicProgress(
      topic: Topic,
      topicProgress: TopicProgress,
    ): Topic {
                  ...
                  TopicPlayAvailability
                    ...
                    .apply {
                      if (topicRecord.isPublished) {
                        ...
                        availableToPlayInFuture = true
                      }
                    }.build()
              }.build()
          }
      } else {
        createTopicFromJson(topicId)
      }
    }
    ...
    private fun fetchChapter(
      ...
      storySummary.chapterList.firstOrNull {
        it.explorationId == explorationId
      }

    internal fun retrieveStory(
      topicId: String,
      storyId: String,
    ): StorySummary =
      if (loadLessonProtosFromAssets) {
        ...
        createStorySummaryFromJson(topicId, storyId)
      }

    // TODO(#45): Expose this as a data provider, or omit if it's not needed.
    private fun retrieveReviewCard(
      topicId: String,
      subtopicId: Int,
    ): RevisionCard = revisionCardRetriever.loadRevisionCard(topicId, subtopicId)

    // Loads and returns the questions given a list of skill ids.
    private fun loadQuestionsForSkillIds(skillIdsList: List<String>): List<Question> = questionRetriever.loadQuestions(skillIdsList)
    ...
    private fun createTopicFromJson(topicId: String): Topic {
      ...
      val topicPlayAvailability =
        if (topicData.getBoolean("published")) {
    ...
    private fun createSubtopicListFromJsonArray(subtopicJsonArray: JSONArray?): List<Subtopic> {
      ...
      for (i in 0 until subtopicJsonArray!!.length()) {
        ...
        val skillIdList =
          currentSubtopicJsonObject.optJSONArray("skill_ids")?.let { skillJsonArray ->
    ...
    private fun getProtoAssetFileNameList(topicId: String): List<String> {
      ...
      val storyRecords =
        topicRecord.canonicalStoryIdsList.map { storyId ->
          assetRepository.loadProtoFromLocalAssets(
            assetName = storyId,
    ...
    internal fun getJsonAssetFileNameList(topicId: String): List<String> {
      ...
      val storyFileNames =
        topicJsonObject?.optJSONArray("canonical_story_dicts")?.let { storyArray ->
          (0 until storyArray.length())
            .mapNotNull(storyArray::optJSONObject)
            .mapNotNull { it.optString("id") }
            .map { "$it.json" }
        } ?: listOf()

      val chapterFileNames =
        storyFileNames.flatMap { storyFileName ->
          ...
          storyJson?.optJSONArray("story_nodes")?.let { storyNodeJsonArray ->
            (0 until storyNodeJsonArray.length())
              .mapNotNull(storyNodeJsonArray::optJSONObject)
              .mapNotNull { it.optString("exploration_id") }
              .map { "$it.json" }
          } ?: listOf()
        }

      val subtopicFileNames =
        topicJsonObject?.optJSONArray("subtopics")?.let { subtopicJsonArray ->
          (0 until subtopicJsonArray.length())
            .mapNotNull(subtopicJsonArray::optJSONObject)
     ...
     * story in json.
     */
    private fun createStorySummaryFromJson(
      topicId: String,
      storyId: String,
    ): StorySummary {
      ...
      val chapterList =
        storyDataJsonObject?.getJSONArray("story_nodes")?.let {
    ...
    private fun createStoryThumbnail(
      topicId: String,
      storyId: String,
    ): LessonThumbnail {
      val topicJsonObject = jsonAssetRetriever.loadJsonFromAsset("$topicId.json")!!
    ...
    private fun Topic.toEphemeral(contentLocale: OppiaLocale.ContentLocale): EphemeralTopic =
      EphemeralTopic
        .newBuilder()
        .apply {
    ...
    private fun StorySummary.toEphemeral(contentLocale: OppiaLocale.ContentLocale): EphemeralStorySummary =
    ...
    private fun ChapterSummary.toEphemeral(contentLocale: OppiaLocale.ContentLocale): EphemeralChapterSummary =
      EphemeralChapterSummary
        ...
        .apply {
          ...
          writtenTranslationContext =
            translationController.computeWrittenTranslationContext(
              chapterSummary.writtenTranslationsMap,
              contentLocale,
            )
    ...
    private fun Subtopic.toEphemeral(contentLocale: OppiaLocale.ContentLocale): EphemeralSubtopic =
      EphemeralSubtopic
        ...
        .apply {
          ...
          writtenTranslationContext =
            translationController.computeWrittenTranslationContext(
              subtopic.writtenTranslationsMap,
              contentLocale,
            )
        }.build()
