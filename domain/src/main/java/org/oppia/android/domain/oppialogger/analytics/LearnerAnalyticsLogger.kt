  ...
  constructor(
    ...
    fun endExploration() {
      if (mutableExpAnalyticsLogger.value == null) {
        oppiaLogger.w(
          "LearnerAnalyticsLogger",
          "Attempting to end an exploration that hasn't been started.",
        )
      }
    ...
    class ExplorationAnalyticsLogger internal constructor(
      ...
      fun startCard(newState: State): StateAnalyticsLogger =
        StateAnalyticsLogger(
          ...
          baseLogger,
          newState,
          explorationLogContext,
        ).also {
          if (!mutableStateAnalyticsLogger.compareAndSet(expect = null, update = it)) {
            oppiaLogger.w(
              "LearnerAnalyticsLogger",
              "Attempting to start a card without ending the previous.",
            )
      ...
      private fun getExpectedStateLogger(): StateAnalyticsLogger? =
        mutableStateAnalyticsLogger.value.also {
          if (it == null) {
    ...
    class StateAnalyticsLogger internal constructor(
      ...
      fun logSubmitAnswer(
        ...
        val stringifiedUserAnswer =
          when (answer.objectTypeCase) {
            ...
            InteractionObject.ObjectTypeCase.OBJECTTYPE_NOT_SET, null,
            -> null
          }
    ...
    internal class BaseLogger internal constructor(
      ...
      fun maybeLogEvent(context: EventContext?) = analyticsController.maybeLogEvent(installationId, context, profileId, oppiaLogger)
    ...
    private companion object {
      ...
      private fun <T : MessageLite> T.ensureNonEmpty(): T? = takeIf { it != it.defaultInstanceForType }

      private fun createLearnerDetailsContext(
        installationId: String?,
        learnerId: String?,
      ): LearnerDetailsContext? =
        createLearnerDetailsContextWithIdsPresent(
          installationId?.takeUnless(String::isEmpty),
          learnerId?.takeUnless(String::isEmpty),
        ).ensureNonEmpty()
      ...
      private fun createSwitchInLessonLanguageContext(
        ...
        toLanguage: OppiaLanguage,
        explorationDetails: ExplorationContext,
      ) = EventLog.SwitchInLessonLanguageEventContext
        .newBuilder()
        .apply {
