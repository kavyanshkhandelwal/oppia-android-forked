...
class PerformanceMetricsController
  ...
  constructor(
    ...
    private fun cacheMetricLog(oppiaMetricLog: OppiaMetricLog) {
      metricLogStore
        .storeDataAsync(updateInMemoryCache = true) { oppiaMetricLogs ->
          ...
          if (storeSize + 1 > metricLogStorageCacheSize) {
            ...
            if (eventLogRemovalIndex != null) {
              return@storeDataAsync oppiaMetricLogs
                .toBuilder()
    ...
    private fun getLeastRecentGeneralEventIndex(oppiaMetricLogs: OppiaMetricLogs): Int? =
      oppiaMetricLogs.oppiaMetricLogList
        .withIndex()
     ...
     */
    suspend fun getMetricLogStoreList(): List<OppiaMetricLog> = metricLogStore.readDataAsync().await().oppiaMetricLogList
