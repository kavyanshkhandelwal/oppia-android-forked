 ...
 */
@Singleton
class QuestionAssessmentProgressController
  @Inject
  constructor(
    ...
    fun getCurrentQuestion(): DataProvider<EphemeralQuestion> {
      ...
      return writtenTranslationContentLocale.combineWith(
        ...
        LOCALIZED_QUESTION_PROVIDER_ID,
    ...
    private suspend fun ControllerState?.finishQuestionTrainingSessionImpl(finishSessionResultFlow: MutableStateFlow<AsyncResult<Any?>>) {
        ...
        try {
          ...
          val classificationResult =
            answerClassificationController.classify(
              ...
              answer.answer,
              answer.writtenTranslationContext,
            )
          ...
          progress.stateDeck.submitAnswer(
            answer,
            answeredQuestionOutcome.feedback,
            answeredQuestionOutcome.isCorrectAnswer,
          )

          // Track the number of answers the user submitted, including any misconceptions
          val misconception =
            if (classificationResult is OutcomeWithMisconception) {
    ...
    private suspend fun ControllerState.moveToNextQuestion(moveToNextQuestionResultFlow: MutableStateFlow<AsyncResult<Any?>>) {
    ...
    private suspend fun ControllerState.retrieveCurrentQuestionAsync(questionsList: List<Question>): AsyncResult<EphemeralQuestion> =
      try {
              ...
              .apply {
                writtenTranslationContext =
                  translationController.computeWrittenTranslationContext(
                    state.writtenTranslationsMap,
                    writtenTranslationContentLocale,
                  )
              }.build()
        }.build()

    private fun ControllerState.retrieveEphemeralQuestionState(questionsList: List<Question>): EphemeralQuestion {
      val currentQuestionIndex = progress.getCurrentQuestionIndex()
      val ephemeralQuestionBuilder =
        EphemeralQuestion
          .newBuilder()
