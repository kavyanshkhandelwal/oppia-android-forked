...
class AnalyticsController
  ...
  constructor(
    ...
    private fun logEvent(
      ...
      CoroutineScope(blockingDispatcher)
        .async {
    ...
    private suspend fun cacheEventLog(eventLog: EventLog) {
      eventLogStore
        .storeDataAsync(updateInMemoryCache = true) { oppiaEventLogs ->
          ...
          if (storeSize + 1 > eventLogStorageCacheSize) {
            ...
            if (eventLogRemovalIndex != null) {
              return@storeDataAsync oppiaEventLogs
                .toBuilder()
    ...
    private fun getLeastRecentEventIndex(oppiaEventLogs: OppiaEventLogs): Int? =
      oppiaEventLogs.eventLogsToUploadList
        .withIndex()
        .filter { it.value.priority == Priority.OPTIONAL }
        .minByOrNull { it.value.timestamp }
        ?.index ?: getLeastRecentGeneralEventIndex(oppiaEventLogs)
    ...
    private fun uploadAllEvents(): Flow<AsyncResult<Pair<Int, Int>>> {
      ...
      return retrieveEventLogCountAsync()
        .transform { eventLogCount ->
    ...
    private fun retrieveEventLogCountAsync(): Flow<Int> = flow { emit(eventLogStore.readDataAsync().await().eventLogsToUploadCount) }
    ...
    private fun removeFirstEventLogFromStoreAsync(): Deferred<EventLog> =
      eventLogStore
        .storeDataWithCustomChannelAsync(updateInMemoryCache = true) { eventLogs ->
          eventLogs
            .toBuilder()
            .apply {
              if (enableLearnerStudyAnalytics) {
