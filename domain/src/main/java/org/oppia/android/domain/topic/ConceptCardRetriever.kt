  ...
  constructor(
    ...
    fun loadConceptCard(skillId: String): ConceptCard {
      val conceptCard =
        if (loadLessonProtosFromAssets) {
          ...
          loadConceptCardFromJson(skillId)
        }
    ...
    private fun loadConceptCardFromJson(skillId: String): ConceptCard? {
      try {
        ...
        val explanationVoiceovers =
          explanationContentId?.let {
            skillContents
        ...
        for (workedExample in workedExamplesList) {
          recordedVoiceoverMapping[workedExample.contentId] =
            createRecordedVoiceoversFromJson(
              skillContents
                .optJSONObject("recorded_voiceovers")
                ?.optJSONObject("voiceovers_mapping")
    ...
    private fun createSubtitledHtml(subtitledHtmlJson: JSONObject): SubtitledHtml =
      SubtitledHtml
        .newBuilder()
        .apply {
