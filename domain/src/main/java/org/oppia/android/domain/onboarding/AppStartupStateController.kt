...
@Singleton
class AppStartupStateController
  @Inject
  constructor(
      ...
      when (currentBuildFlavor) {
        ...
        BuildFlavor.BETA -> {
          ...
          if (!onboardingState.permanentlyDismissedBetaNotice &&
            ...
            BuildFlavorNoticeMode.NO_NOTICE
          }
        }
        BuildFlavor.GENERAL_AVAILABILITY ->
          when (onboardingState.lastUsedBuildFlavor) {
            BuildFlavor.ALPHA, BuildFlavor.BETA, null -> {
              if (!onboardingState.permanentlyDismissedGaUpgradeNotice) {
                ...
                BuildFlavorNoticeMode.NO_NOTICE // The user doesn't want to see the notice again.
              }
            }
            ...
            BuildFlavor.DEVELOPER, BuildFlavor.GENERAL_AVAILABILITY,
            -> BuildFlavorNoticeMode.NO_NOTICE
          }
      }
    ...
    private fun hasAppExpired(): Boolean {
      ...
      val isAppExpirationEnabled =
        applicationMetadata?.getBoolean(
          "automatic_app_expiration_enabled", // defaultValue=
          true,
        ) ?: true
