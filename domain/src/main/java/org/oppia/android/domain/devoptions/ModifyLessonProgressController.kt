...
@Singleton
class ModifyLessonProgressController
  @Inject
  constructor(
    ...
    fun getAllTopicsWithProgress(profileId: ProfileId): DataProvider<List<EphemeralTopic>> {
      ...
      return topicListProvider.transformAsync(GET_ALL_TOPICS_COMBINED_PROVIDER_ID) { topicList ->
        topicController
          .getTopics(
            profileId,
            topicList.topicSummaryList.map { it.topicSummary.topicId },
          ).retrieveData()
      }
    }
     ...
     */
    fun getStoryMapWithProgress(profileId: ProfileId): DataProvider<Map<String, List<EphemeralStorySummary>>> =
      getAllTopicsWithProgress(profileId).transform(GET_ALL_STORIES_PROVIDER_ID) { topics ->
     ...
     */
    fun markMultipleTopicsCompleted(
      profileId: ProfileId,
      topicIdList: List<String>,
    ) {
      topicIdList.forEach { topicId ->
        val topic =
          checkNotNull(topicController.retrieveTopic(topicId)) {
