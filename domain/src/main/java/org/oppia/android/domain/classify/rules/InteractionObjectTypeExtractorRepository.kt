...
class InteractionObjectTypeExtractorRepository
  @Inject
  constructor() {
    ...
    private companion object {
      private fun computeExtractorMap(): Map<ObjectTypeCase, ExtractorMapping<*>> =
        ObjectTypeCase.values().associate {
          it to computeExtractorMapping(it)
        }
      ...
      private inline fun <reified T : Any> createMapping(noinline extractor: (InteractionObject) -> T): ExtractorMapping<T> =
        ExtractorMapping(T::class, extractor)
    }
  }
