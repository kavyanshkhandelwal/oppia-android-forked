...
class ProfileManagementController
  ...
  constructor(
    ...
    class ProfileNotAdminException(
      msg: String,
    ) : Exception(msg)

    /** Indicates that the Profile already has admin. */
    class ProfileAlreadyHasAdminException(
      msg: String,
    ) : Exception(msg)
    ...
    class DeviceSettingsNotFoundException(
      msg: String,
    ) : Exception(msg)
    ...
    fun getProfiles(): DataProvider<List<Profile>> =
      profileDataStore.transform(GET_PROFILES_PROVIDER_ID) {
    ...
    fun getProfile(profileId: ProfileId): DataProvider<Profile> =
      profileDataStore.transformAsync(GET_PROFILE_PROVIDER_ID) {
    ...
    fun getWasProfileEverAdded(): DataProvider<Boolean> =
      profileDataStore.transformAsync(GET_WAS_PROFILE_EVER_ADDED_PROVIDER_ID) {
    ...
    fun getDeviceSettings(): DataProvider<DeviceSettings> =
      profileDataStore.transformAsync(GET_DEVICE_SETTINGS_PROVIDER_ID) {
    ...
    fun addProfile(
      ...
      val deferred =
        profileDataStore.storeDataWithCustomChannelAsync(
    ...
    private fun computeProfileType(
      isAdmin: Boolean,
      pin: String?,
    ): ProfileType =
      when {
    ...
    private fun isAdminWithPin(
      isAdmin: Boolean,
      pin: String?,
    ): Boolean = isAdmin && !pin.isNullOrBlank()
    ...
    fun getProfileOnboardingMode(): DataProvider<ProfileOnboardingMode> =
      getProfiles().transform(PROFILE_ONBOARDING_MODE_PROVIDER_ID) { profileList ->
    ...
    fun getAudioLanguage(profileId: ProfileId): DataProvider<AudioLanguage> =
      translationController
        .getAudioTranslationContentLanguage(
          profileId,
          ...
          when (oppiaLanguage) {
            ...
            OppiaLanguage.PORTUGUESE, OppiaLanguage.SWAHILI,
    ...
    fun loginToProfile(profileId: ProfileId): DataProvider<Any?> {
      return setCurrentProfileId(profileId)
        .transformAsync(LOGIN_TO_PROFILE_PROVIDER_ID) {
    ...
    private fun updateLastLoggedInAsyncAndNumberOfLogins(profileId: ProfileId): Deferred<ProfileActionStatus> {
      return profileDataStore.storeDataWithCustomChannelAsync(updateInMemoryCache = true) {
        val profile =
          it.profilesMap[profileId.internalId]
    ...
    fun getCurrentProfileId(): ProfileId? =
      currentProfileId.takeIf { it != DEFAULT_LOGGED_OUT_INTERNAL_PROFILE_ID }?.let {
    ...
    suspend fun markContinueButtonAnimationSeen(profileId: ProfileId) {
      val updateDatabaseDeferred =
        profileDataStore.storeDataAsync(true) {
          val profile = it.profilesMap[profileId.internalId]
          if (profile != null) {
      ...
      val deferred =
        profileDataStore.storeDataWithCustomChannelAsync(
          ...
          val updatedProfile =
            profile
              ?.toBuilder()
              ?.setSurveyLastShownTimestampMs(
                oppiaClock.getCurrentTimeMs(),
              )?.build()
          val profileDatabaseBuilder =
            profileDatabase.toBuilder().putProfiles(
              profileId.internalId,
              updatedProfile,
            )
          Pair(profileDatabaseBuilder.build(), ProfileActionStatus.SUCCESS)
    ...
    fun retrieveSurveyLastShownTimestamp(profileId: ProfileId): DataProvider<Long> =
      profileDataStore.transformAsync(RETRIEVE_SURVEY_LAST_SHOWN_TIMESTAMP_PROVIDER_ID) {
     ...
     * [profileId].
     */
    fun retrieveLastSelectedClassroomId(profileId: ProfileId): DataProvider<String?> =
      profileDataStore.transformAsync(RETRIEVE_LAST_SELECTED_CLASSROOM_ID_PROVIDER_ID) {
    ...
    private fun rotateAndCompressBitmap(
      uri: Uri,
      bitmap: Bitmap,
      cropSize: Int,
    ): Bitmap {
