...
class LocaleController
  ...
  constructor(
    ...
    private fun getSystemLocaleProfile(): DataProvider<AndroidLocaleProfile> =
      dataProviders.createInMemoryDataProvider(ANDROID_SYSTEM_LOCALE_DATA_PROVIDER_ID) {
    ...
    private fun getSystemLocale(): Locale =
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
        ...
        getDefaultLocale(applicationContext.resources.configuration)
      }
     ...
     */
    private suspend fun retrieveLanguageDefinition(language: OppiaLanguage): LanguageSupportDefinition? {
      val definitions = retrieveAllLanguageDefinitions()
      return definitions.languageDefinitionsList
        .find {
     ...
     * system languages.
     */
    private suspend fun retrieveLanguageDefinitionFromSystemCode(localeProfile: AndroidLocaleProfile): LanguageSupportDefinition? {
      val definitions = retrieveAllLanguageDefinitions()
      ...
      return definitions.languageDefinitionsList
        ...
        }.find { (profile, _) -> localeProfile.matches(profile) }
    ...
    private suspend fun retrieveAllLanguageDefinitions() =
    ...
    @Suppress("RedundantSuspendModifier") // Keep to force calls to background threads.
    private suspend fun retrieveAllRegionDefinitions() =
      definitionsLock.withLock {
    ...
    private fun LanguageSupportDefinition.retrieveAppLanguageProfile(): AndroidLocaleProfile? =
      when (appStringId.languageTypeCase) {
    ...
    private fun computeDefaultLanguageDefinitionForSystemLanguage(systemLocaleProfile: AndroidLocaleProfile) =
      LanguageSupportDefinition
        .newBuilder()
        .apply {
    ...
    private fun computeDefaultLanguageDefinitionForContentStrings(): LanguageSupportDefinition {
      ...
      return LanguageSupportDefinition
        .newBuilder()
        .apply {
          language = OppiaLanguage.LANGUAGE_UNSPECIFIED
          minAndroidSdkVersion = 1 // Assume it's supported on the current version.
