...
class DeprecationController
  ...
  constructor(
    ...
    fun saveDeprecationResponse(deprecationResponse: DeprecationResponse): DataProvider<Any?> {
      val deferred =
        deprecationStore.storeDataWithCustomChannelAsync(
          updateInMemoryCache = true,
        ) { deprecationResponseDb ->
          val deprecationBuilder =
            deprecationResponseDb
              .toBuilder()
              .apply {
     ...
     * @return [AsyncResult]
     */
    private suspend fun getDeferredResult(deferred: Deferred<DeprecationResponseActionStatus>): AsyncResult<Any?> =
      when (deferred.await()) {
