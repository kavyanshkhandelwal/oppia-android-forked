...
@Singleton
class LoggingIdentifierController
  @Inject
  constructor(
    ...
    private val installationIdStore by lazy {
      persistentCacheStoreFactory
        .create(
          cacheName = "device_context_database",
          DeviceContextDatabase.getDefaultInstance(),
        ).also {
          it
            .primeInMemoryAndDiskCacheAsync(
              ...
              if (failure != null) {
                oppiaLogger.e(
                  ...
                  "Failed to initialize the installation ID",
                  failure,
                )
              }
            }
    ...
    fun getInstallationId(): DataProvider<String> =
      installationIdStore.transform(
        INSTALLATION_ID_DATA_PROVIDER_ID,
        DeviceContextDatabase::getInstallationId,
      )
     ...
     */
    private fun Random.randomUuid(): UUID = UUID.nameUUIDFromBytes(ByteArray(16).also { this@randomUuid.nextBytes(it) })
  }
