...
@Singleton
class PlatformParameterController
  @Inject
  constructor(
    ...
    private val platformParameterDatabaseStore =
     ...
     */
    fun updatePlatformParameterDatabase(platformParameterList: List<PlatformParameter>): DataProvider<Any?> {
      val deferredTask =
        platformParameterDatabaseStore.storeDataWithCustomChannelAsync(
     ...
     */
    private suspend fun getDeferredResult(deferred: Deferred<PlatformParameterCachingStatus>): AsyncResult<Any?> =
      when (deferred.await()) {
