...
@Singleton
class SurveyProgressController
  @Inject
  constructor(
    ...
    fun getCurrentQuestion(): DataProvider<EphemeralSurveyQuestion> {
      ...
      val questionsListDataProvider =
        monitoredQuestionListDataProvider.combineWith(
          ephemeralQuestionDataProvider,
          EPHEMERAL_QUESTION_FROM_UPDATED_QUESTION_LIST_PROVIDER_ID,
        ) { _, currentQuestion ->
          currentQuestion
        }
     ...
     *     the user.
     */
    fun endSurveySession(surveyCompleted: Boolean): DataProvider<Any?> {
      ...
      val message =
        ControllerMessage.FinishSurveySession(
          surveyCompleted,
          activeSessionId,
    ...
    private fun createCurrentQuestionDataProvider(
      ...
      questionsListDataProvider.transformNested(MONITORED_QUESTION_LIST_PROVIDER_ID) {
        maybeSendReceiveQuestionListEvent(commandQueue = null, it)
      }
    ...
    private fun createEmptyQuestionsListDataProvider(): DataProvider<List<SurveyQuestion>> =
      dataProviders.createInMemoryDataProvider(EMPTY_QUESTIONS_LIST_DATA_PROVIDER_ID) {
        listOf()
      }
    ...
    private suspend fun ControllerState.beginSurveySessionImpl(beginSessionResultFlow: MutableStateFlow<AsyncResult<Any?>>) {
    ...
    private fun ControllerState.saveSelectedAnswer(
      questionId: String,
    ...
    private fun recordSelectedAnswerAsync(
      ...
      answerDataStore.storeDataWithCustomChannelAsync(
    ...
    private suspend fun ControllerState.moveToNextQuestionImpl(moveToNextQuestionResultFlow: MutableStateFlow<AsyncResult<Any?>>) {
    ...
    private suspend fun ControllerState.moveToPreviousQuestionImpl(moveToPreviousQuestionResultFlow: MutableStateFlow<AsyncResult<Any?>>) {
    ...
    private suspend fun ControllerState.completeSurveyImpl(endSessionResultFlow: MutableStateFlow<AsyncResult<Any?>>) {
    ...
    private fun <T> createAsyncResultStateFlow(initialValue: AsyncResult<T> = AsyncResult.Pending()) = MutableStateFlow(initialValue)

    private fun <T> StateFlow<AsyncResult<T>>.convertToSessionProvider(baseId: String): DataProvider<T> =
      dataProviders.run {
    ...
    private suspend fun ControllerState.finishSurveyAndLog() {
      when {
        progress.questionDeck.hasAnsweredAllMandatoryQuestions() -> {
          ...
          val currentQuestionName =
            progress.questionGraph
