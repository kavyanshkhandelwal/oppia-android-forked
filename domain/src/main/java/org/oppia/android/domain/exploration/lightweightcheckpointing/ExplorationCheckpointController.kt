 ...
 */
@Singleton
class ExplorationCheckpointController
  @Inject
  constructor(
    ...
    class ExplorationCheckpointNotFoundException(
      message: String,
    ) : Exception(message)

    /** Indicates that no checkpoint was found for the specified explorationId and profileId. */
    class OutdatedExplorationCheckpointException(
      message: String,
    ) : Exception(message)
    ...
    fun recordExplorationCheckpoint(
      ...
      val deferred =
        recordExplorationCheckpointAsync(
          profileId,
          explorationId,
          explorationCheckpoint,
        )
          ...
          when {
            ...
            checkpoint != null && exploration.version != checkpoint.explorationVersion -> {
              ...
              if (migratedCheckpoint == null) {
                ...
                AsyncResult.Success(migratedCheckpoint)
              }
            }
            else -> {
              AsyncResult.Failure(
     ...
     */
    fun retrieveOldestSavedExplorationCheckpointDetails(profileId: ProfileId): DataProvider<ExplorationCheckpointDetails> =
      retrieveCacheStore(profileId)
        .transform(
          ...
          explorationCheckpointDatabase.explorationCheckpointMap
            .minByOrNull {
              it.value.timestampOfFirstCheckpoint
            }?.let { (expId, exp) ->
              ExplorationCheckpointDetails
                .newBuilder()
                .apply {
                  explorationId = expId
    ...
    private fun retrieveCacheStore(profileId: ProfileId): PersistentCacheStore<ExplorationCheckpointDatabase> {
      val cacheStore =
    ...
    private fun replayCheckpoint(
      ...
      val nextStateNames =
        completedStates.indices.map {
          completedStates.getOrNull(it + 1)?.stateName ?: checkpoint.pendingStateName
        }
      val updatedCompletedStates =
        completedStates.mapIndexed { idx, completedState ->
      ...
      val updatedPendingAnswers =
        replayPendingState(
          ...
          checkpoint.pendingUserAnswersList,
          exploration,
        ) ?: return null
    ...
    private fun replayCompletedState(
      ...
      val completedAnswers =
        completedState.answerList.mapIndexed { idx, answer ->
      ...
      return completedCheckpointState
        .toBuilder()
        .apply {
          this.completedState =
            completedState
