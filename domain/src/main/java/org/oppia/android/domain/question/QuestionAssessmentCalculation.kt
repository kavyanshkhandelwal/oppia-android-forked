...
internal class QuestionAssessmentCalculation private constructor(
  ...
  private fun computeScoresPerSkill(): Map<String, FractionGrade> =
    questionSessionMetrics
      .flatMap { questionMetric ->
        // Convert to List<Pair<String, QuestionSessionMetrics>>>.
        questionMetric.question.linkedSkillIdsList
          .filter { skillId ->
  ...
  private fun QuestionSessionMetrics.computeQuestionScore(): Int =
    if (!didViewSolution) {
  ...
  private fun QuestionSessionMetrics.getAdjustedWrongAnswerCount(): Int = (numberOfAnswersSubmitted - 1).coerceAtLeast(0)
