...
@Singleton
class ExplorationActiveTimeController
  @Inject
  constructor(
    ...
    override fun onExplorationStarted(
      profileId: ProfileId,
      topicId: String,
    ) {
    ...
    private fun stopSessionTimerAsync(isExplorationStarted: Boolean): DataProvider<Any?> {
      ...
      val message =
        ControllerMessage.StopSessionTimer(
    ...
    private fun pauseSessionTimerAsync(): DataProvider<Any?> {
      ...
      val message =
        ControllerMessage.PauseSessionTimer(
          sessionId = activeSessionId,
    ...
    private fun resumeSessionTimer(isExplorationStarted: Boolean): DataProvider<Any?> {
      ...
      val message =
        ControllerMessage.ResumeSessionTimer(
            ...
            .apply {
              val topicLearningTime =
                aggregateTopicLearningTimeMap
                  ...
                  .apply {
                    topicLearningTimeMs =
                      if (isLastUpdatedTimestampStale(lastUpdatedTimeMs)) {
                        sessionDuration
                      } else {
    ...
    private suspend fun getDeferredResult(deferred: Deferred<TopicLearningTimeActionStatus>): AsyncResult<Any?> =
      when (deferred.await()) {
        TopicLearningTimeActionStatus.SUCCESS -> AsyncResult.Success(null)
      }

    private fun retrieveCacheStore(profileId: ProfileId): PersistentCacheStore<TopicLearningTimeDatabase> =
      cacheStoreMap.getOrPut(profileId) {
        cacheStoreFactory
          .createPerProfile(
            CACHE_NAME,
