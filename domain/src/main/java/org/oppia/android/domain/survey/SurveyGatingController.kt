...
class SurveyGatingController
  ...
  constructor(
    ...
    private val minimumLearningTimeForGatingMillis =
    ...
    private fun isWithinSurveyTimeWindow(): Boolean =
      when (machineLocale.getCurrentTimeOfDay()) {
    ...
    private fun retrieveSurveyLastShownDate(profileId: ProfileId) = profileManagementController.retrieveSurveyLastShownTimestamp(profileId)

    private fun hasReachedMinimumTopicLearningThreshold(topicLearningTimeMs: Long): Boolean =
      topicLearningTimeMs >= minimumLearningTimeForGatingMillis
    ...
    private fun retrieveAggregateLearningTime(
      ...
      activeTimeController
        .retrieveAggregateTopicLearningTimeDataProvider(
          profileId,
          topicId,
        ).transform(
          GET_TOPIC_LEARNING_TIME_PROVIDER_ID,
