...
object EspressoTestsMatchers {
  fun matchCurrentTabTitle(tabTitle: String): Matcher<View> = matchCurrentTabTitleCheck(tabTitle)

  fun withDrawable(resourceId: Int): Matcher<View> = DrawableMatcher(resourceId)

  @Suppress("unused")
  fun noDrawable(): Matcher<View> = DrawableMatcher(DrawableMatcher.NONE)

  fun <T : MessageLite> hasProtoExtra(
    keyName: String,
    expectedProto: T,
  ): Matcher<Intent> = hasProtoExtraCheck(keyName, expectedProto)
}
