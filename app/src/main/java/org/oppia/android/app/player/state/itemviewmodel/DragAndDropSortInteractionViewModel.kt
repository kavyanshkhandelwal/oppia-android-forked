...
enum class DragAndDropSortInteractionError(
...
class DragAndDropSortInteractionViewModel private constructor(
  ...
  fun getGroupingStatus(): Boolean = allowMultipleItemsInSamePosition
  ...
  private fun getSubmitTimeError(): DragAndDropSortInteractionError {
    val haveItemsChanged =
      _originalChoiceItems.size != _choiceItems.size ||
  ...
  companion object {
    private fun computeOriginalChoiceItems(
      ...
      choiceStrings
        .mapIndexed { index, subtitledHtml ->
          DragDropInteractionContentViewModel(
            contentIdHtmlMap = contentIdHtmlMap,
            htmlContent =
              SetOfTranslatableHtmlContentIds
  ...
  private fun computeSelectedChoiceItems(
    ...
    val explorationEphemeralStateLiveData =
      MediatorLiveData<AsyncResult<EphemeralState>>().apply {
    ...
    choiceItems =
      Transformations.map(explorationEphemeralStateLiveData) { result ->
        when (result) {
  ...
  private fun processEphemeralStateResult(
    ...
    return if (wrongAnswerList.isNotEmpty()) {
      val latestWrongAnswerContentIdList =
        wrongAnswerList
          .last()
          .userAnswer.answer.listOfSetsOfTranslatableHtmlContentIds.contentIdListsList
      latestWrongAnswerContentIdList
        .mapIndexed { index, setOfTranslatableHtmlContentIds ->
          DragDropInteractionContentViewModel(
            contentIdHtmlMap = contentIdHtmlMap,
            htmlContent =
              SetOfTranslatableHtmlContentIds
