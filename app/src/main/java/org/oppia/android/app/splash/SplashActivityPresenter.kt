...
class SplashActivityPresenter
  ...
  constructor(
    ...
    fun handleOnCreate() {
      DataBindingUtil
        .setContentView<SplashActivityBinding>(
          activity,
    ...
    fun handleOnDeprecationNoticeActionClicked(noticeActionResponse: DeprecationNoticeActionResponse) {
      when (noticeActionResponse) {
        ...
        is DeprecationNoticeActionResponse.Dismiss ->
          handleOnDeprecationNoticeDialogDismissed(
    ...
    private fun resumeOnboarding(
      profileId: ProfileId,
      profileName: String,
    ) {
      val introActivityParams =
        IntroActivityParams
          .newBuilder()
    ...
    private fun computeInitStateDataProvider(): DataProvider<SplashInitState> {
      ...
      return startupStateDataProvider.combineWith(
        systemAppLanguageLocaleDataProvider,
        SPLASH_INIT_STATE_DATA_PROVIDER_ID,
