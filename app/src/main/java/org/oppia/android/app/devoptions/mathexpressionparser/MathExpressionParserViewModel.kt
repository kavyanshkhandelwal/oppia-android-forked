 ...
 */
@FragmentScope
class MathExpressionParserViewModel
  @Inject
  constructor(
    ...
    private fun updateParseResult() {
      ...
      parseResultTextView.text =
        if ("oppia-noninteractive-math" in newText) {
      ...
      val parseResult =
        when (parseType) {
          ...
          ParseType.ALGEBRAIC_EXPRESSION -> {
            MathExpressionParser
              .parseAlgebraicExpression(expression, allowedVariables)
              .transformExpression(resultType, useDivAsFractions, mathExpressionAccessibilityUtil)
          }
          ParseType.ALGEBRAIC_EQUATION -> {
            MathExpressionParser
              .parseAlgebraicEquation(expression, allowedVariables)
    ...
    private companion object {
      ...
      private fun MathParsingResult<MathExpression>.transformExpression(
        ...
        when (resultType) {
          ...
          ResultType.HUMAN_READABLE_STRING ->
            map {
