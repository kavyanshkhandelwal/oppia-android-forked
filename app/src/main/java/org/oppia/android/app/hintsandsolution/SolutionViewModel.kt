...
class SolutionViewModel private constructor(
  ...
  private val correctAnswerTextStringResId =
    if (isSolutionExclusive) {
      R.string.hints_list_exclusive_solution_text
    } else {
      R.string.hints_list_possible_solution_text
    }

  private fun computeCorrectAnswerHtml(): String {
    val answerTextHtml =
      when (correctAnswer.objectTypeCase) {
  ...
  private fun String.toNumericExpressionHtml() = parseNumericExpression().expectSuccess().toRawLatex().wrapAsLatexHtml()

  private fun String.toReadableNumericExpression() = parseNumericExpression().expectSuccess().toReadableString() ?: this

  private fun String.toAlgebraicExpressionHtml() = parseAlgebraicExpression().expectSuccess().toRawLatex().wrapAsLatexHtml()

  private fun String.toReadableAlgebraicExpression() = parseAlgebraicExpression().expectSuccess().toReadableString() ?: this

  private fun String.toAlgebraicEquationHtml() = parseAlgebraicEquation().expectSuccess().toRawLatex().wrapAsLatexHtml()

  private fun String.toReadableAlgebraicEquation() = parseAlgebraicEquation().expectSuccess().toReadableString() ?: this
  ...
  private fun MathExpression.toRawLatex() = toRawLatex(divAsFraction = interaction.extractUseFractionsForDivision())

  private fun MathEquation.toRawLatex() = toRawLatex(divAsFraction = interaction.extractUseFractionsForDivision())
  ...
  private fun Double.toSimplifiedReadableString(): String {
    ...
    return if (isApproximatelyEqualTo(longPart.toDouble())) {
      ...
      appLanguageResourceHandler.formatDouble(this)
    }
  }
  ...
  private companion object {
    ...
    private fun String.parseNumericExpression() = parseNumericExpression(rawExpression = this, errorCheckingMode = REQUIRED_ONLY)
    ...
    private fun Interaction.extractUseFractionsForDivision() = customizationArgsMap["useFractionForDivision"]?.boolValue ?: false
  }
