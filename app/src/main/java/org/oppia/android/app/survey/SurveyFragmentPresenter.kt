  ...
  constructor(
    ...
    private fun processEphemeralQuestion(ephemeralQuestion: EphemeralSurveyQuestion) {
      ...
      when (questionName) {
        ...
        SurveyQuestionName.PROMOTER_FEEDBACK ->
          surveyViewModel.itemList.add(
        ...
        SurveyQuestionName.DETRACTOR_FEEDBACK ->
          surveyViewModel.itemList.add(
            FreeFormItemsViewModel(
              answerAvailabilityReceiver,
              questionName,
    ...
    private fun getPreviousAnswerHandler(itemList: List<SurveyAnswerItemViewModel>): PreviousAnswerHandler? =
      itemList.findLast { it is PreviousAnswerHandler } as? PreviousAnswerHandler
        ...
        { result ->
          when (result) {
            is AsyncResult.Failure -> {
              oppiaLogger.e(
                "SurveyFragment",
                "Failed to submit free form answer",
                result.error,
              )
            }
    ...
    private fun toggleNavigationButtonVisibility(
      questionIndex: Int,
      questionCount: Int,
    ) {
      when (questionIndex) {
