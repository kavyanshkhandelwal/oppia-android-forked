 ...
 * instance).
 */
class AppLanguageResourceHandler
  @Inject
  constructor(
    ...
    fun formatInLocaleWithWrapping(
      format: String,
      vararg formatArgs: String,
    ): String = getDisplayLocale().run { format.formatInLocaleWithWrapping(*formatArgs) }

    /** See [OppiaLocale.DisplayLocale.formatInLocaleWithoutWrapping]. */
    fun formatInLocaleWithoutWrapping(
      format: String,
      vararg formatArgs: String,
    ): String = getDisplayLocale().run { format.formatInLocaleWithoutWrapping(*formatArgs) }

    /** See [OppiaLocale.DisplayLocale.capitalizeForHumans]. */
    fun capitalizeForHumans(str: String): String = getDisplayLocale().run { str.capitalizeForHumans() }
    ...
    fun getStringInLocale(
      @StringRes id: Int,
    ): String = getDisplayLocale().run { resources.getStringInLocale(id) }
    ...
    ): String = getDisplayLocale().run { resources.getStringInLocaleWithWrapping(id, *formatArgs) }
    ...
    fun getStringInLocaleWithoutWrapping(
      @StringRes id: Int,
      vararg formatArgs: CharSequence,
    ): String = getDisplayLocale().run { resources.getStringInLocaleWithoutWrapping(id, *formatArgs) }
    ...
    fun getStringArrayInLocale(
      @ArrayRes id: Int,
    ): List<String> = getDisplayLocale().run { resources.getStringArrayInLocale(id) }
    ...
    fun getQuantityStringInLocale(
      @PluralsRes id: Int,
      quantity: Int,
    ): String = getDisplayLocale().run { resources.getQuantityStringInLocale(id, quantity) }
    ...
    fun toHumanReadableString(number: Int): String = getDisplayLocale().toHumanReadableString(number)

    /** See [OppiaLocale.DisplayLocale.computeDateString]. */
    fun computeDateString(timestampMillis: Long): String = getDisplayLocale().computeDateString(timestampMillis)

    /** See [OppiaLocale.DisplayLocale.computeDateTimeString]. */
    fun computeDateTimeString(timestampMillis: Long): String = getDisplayLocale().computeDateTimeString(timestampMillis)
    ...
    private fun getLocalizedDisplayName(
      languageCode: String,
      regionCode: String = "",
    ): String {
