...
class TextInputViewModel private constructor(
...
) : StateItemViewModel(ViewType.TEXT_INPUT_INTERACTION),
  ...
  private enum class TextParsingUiError(
    ...
    fun createForText(resourceHandler: AppLanguageResourceHandler): String? = error?.let(resourceHandler::getStringInLocale)

    companion object {
      /** Returns the [TextParsingUiError] corresponding to the input. */
      fun createForText(text: String): TextParsingUiError = if (text.isEmpty()) EMPTY_INPUT else VALID
    }
  }
}
