...
@ActivityScope
class RevisionCardActivityPresenter
  @Inject
  constructor(
    ...
    fun handleOnCreate(
      ...
      val binding =
        DataBindingUtil.setContentView<RevisionCardActivityBinding>(
          activity,
    ...
    private fun processSubtopicTitleLiveData(): LiveData<String> =
      Transformations.map(revisionCardResultLiveData, ::processSubtopicTitleResult)

    private fun processSubtopicTitleResult(revisionCardResult: AsyncResult<EphemeralRevisionCard>): String {
    ...
    fun loadRevisionCardFragment(readingTextSize: ReadingTextSize) {
      if (getReviewCardFragment() != null) {
        activity.supportFragmentManager
          .beginTransaction()
          .remove(getReviewCardFragment() as Fragment)
          .commitNow()
      }
