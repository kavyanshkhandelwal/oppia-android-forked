...
class StateViewModel
  ...
  constructor(
    ...
    fun getPendingAnswer(retrieveAnswerHandler: (List<StateItemViewModel>) -> InteractionAnswerHandler?): UserAnswer? =
      getPendingAnswerWithoutError(
    ...
    fun getUserAnswerState(retrieveAnswerHandler: (List<StateItemViewModel>) -> InteractionAnswerHandler?): UserAnswerState =
      retrieveAnswerHandler(getAnswerItemList())?.getUserAnswerState()
    ...
    private fun getPendingAnswerWithoutError(answerHandler: InteractionAnswerHandler?): UserAnswer? =
      if (answerHandler?.checkPendingAnswerError(AnswerErrorCategory.SUBMIT_TIME) == null) {
    ...
    private fun processIsCurrentLanguageSwahili(languageResult: AsyncResult<OppiaLanguage>): Boolean =
      when (languageResult) {
        ...
        is AsyncResult.Failure -> {
          oppiaLogger.e(
            "StateViewModel",
            "Failed to retrieve content language.",
    ...
    private fun processAllowInLessonQuickLanguageSwitching(profileResult: AsyncResult<Profile>): Boolean =
      when (profileResult) {
