...
class AudioViewModel
  ...
  constructor(
      ...
      when {
        ...
        languages.isNotEmpty() -> {
          ...
          audioPlayerController.changeDataSource(
            voiceOverToUri(voiceoverMap[ensuredLanguageCode]),
            currentContentId,
            ensuredLanguageCode,
          )
        }
    ...
    fun setAudioLanguageCode(languageCode: String) {
      ...
      audioPlayerController.changeDataSource(
        voiceOverToUri(voiceoverMap[languageCode]),
        currentContentId,
        languageCode,
      )
    }
    ...
    fun computeAudioUnavailabilityString(languageName: String): String =
      resourceHandler.getStringInLocaleWithWrapping(
        R.string.audio_unavailable_in_selected_language,
        languageName,
      )
    ...
    private fun processDurationLiveData(): LiveData<Int> = Transformations.map(playProgressResultLiveData, ::processDurationResultLiveData)

    private fun processPositionLiveData(): LiveData<Int> = Transformations.map(playProgressResultLiveData, ::processPositionResultLiveData)

    private fun processPlayStatusLiveData(): LiveData<UiAudioPlayStatus> =
      Transformations.map(playProgressResultLiveData, ::processPlayStatusResultLiveData)
    ...
    private fun voiceOverToUri(voiceover: Voiceover?): String =
      "https://storage.googleapis.com/$gcsResource/exploration/$explorationId/" +
