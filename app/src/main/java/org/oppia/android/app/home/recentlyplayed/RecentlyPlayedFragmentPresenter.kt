...
@FragmentScope
class RecentlyPlayedFragmentPresenter
  @Inject
  constructor(
    ...
    fun handleCreateView(
      ...
      val recentlyPlayedViewModel =
        recentlyPlayedViewModelFactory.create(
    ...
    private fun createLayoutManager(adapter: BindableAdapter<RecentlyPlayedItemViewModel>): RecyclerView.LayoutManager {
      ...
      layoutManager.spanSizeLookup =
        object : GridLayoutManager.SpanSizeLookup() {
          override fun getSpanSize(position: Int): Int =
            if (adapter.getItemViewType(position) == ViewType.VIEW_TYPE_TITLE.ordinal) {
              spanCount
            } else {
              1
            }
    ...
    fun promotedStoryClicked(promotedStory: PromotedStory) {
      val canHavePartialProgressSaved =
        when (promotedStory.chapterPlayState) {
          ...
          ChapterPlayState.COMPLETED, null,
          -> false
        }
    ...
    private fun createRecyclerViewAdapter(): BindableAdapter<RecentlyPlayedItemViewModel> =
      multiTypeBuilderFactory
        .create<RecentlyPlayedItemViewModel, ViewType> { viewModel ->
          when (viewModel) {
    ...
    private fun playExploration(
      ...
      val startPlayingProvider =
        if (canHavePartialProgressSaved) {
