...
class FakeAssetRepository
  ...
  constructor(
    ...
    override fun <T : MessageLite> loadProtoFromLocalAssets(
      assetName: String,
      baseMessage: T,
    ): T = loadProtoFile(assetName, baseMessage) ?: error("Asset doesn't exist: $assetName")
    ...
    override fun loadRemoteBinaryAsset(url: String): () -> ByteArray = error("Loading remote binary files not supported (for URL: $url).")

    override fun loadImageAssetFromLocalAssets(url: String): () -> ByteArray = error("Loading remote images not supported (for URL: $url).")

    override fun primeRemoteBinaryAsset(url: String) = error("Remote binary file priming not supported (for URL: $url).")
    ...
    private fun <T : MessageLite> loadProtoFile(
      assetName: String,
      defaultMessage: T,
    ): T? =
      trackedAssets
        .computeIfAbsent(assetName) {
