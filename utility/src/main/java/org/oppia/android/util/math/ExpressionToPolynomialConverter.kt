  ...
  companion object {
    ...
    private fun createConstantPolynomial(constant: Real): Polynomial =
      createSingleTermPolynomial(
        Polynomial.Term
          .newBuilder()
          .setCoefficient(constant)
          .build(),
      )

    private fun createSingleTermPolynomial(term: Polynomial.Term): Polynomial = Polynomial.newBuilder().apply { addTerm(term) }.build()
  }
}
