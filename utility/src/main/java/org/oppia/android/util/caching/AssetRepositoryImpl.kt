...
class AssetRepositoryImpl
  ...
  constructor(
    ...
    override fun <T : MessageLite> loadProtoFromLocalAssets(
      assetName: String,
      baseMessage: T,
    ): T =
      maybeLoadProtoFromLocalAssets(assetName, baseMessage)
        ?: error("Asset doesn't exist: $assetName")

    override fun <T : MessageLite> tryLoadProtoFromLocalAssets(
      assetName: String,
      defaultMessage: T,
    ): T = maybeLoadProtoFromLocalAssets(assetName, defaultMessage) ?: defaultMessage
    ...
    override fun getLocalAssetProtoSize(assetName: String): Int = loadProtoBlobFromLocalAssets(assetName)?.size ?: -1
    ...
    private fun primeProtoBlobFromLocalAssets(assetName: String) {
      repositoryLock.withLock {
        if (assetName !in protoFileAssets) {
          ...
          protoFileAssets[assetName] =
            if (assetNameFile in files) {
    ...
    override fun isRemoteBinaryAssetDownloaded(url: String): Boolean = getLocalCacheFile(url).exists()

    private fun openRemoteStream(url: String): InputStream = URL(url).openStream()
    ...
    private fun openCachingStreamToRemoteFile(url: String): InputStream {
      ...
      return object : InputStream() {
        override fun available(): Int = urlInStream.available()
    ...
    private fun saveLocalCacheFile(
      identifier: String,
      contents: ByteArray,
    ) {
    ...
    private fun openLocalCacheFileForWrite(identifier: String): OutputStream = getLocalCacheFile(identifier).outputStream()

    private fun getLocalCacheFile(identifier: String): File = File(context.cacheDir, convertIdentifierToCacheFileName(identifier))

    private fun convertIdentifierToCacheFileName(identifier: String): String = "${identifier.hashCode()}.cache"
  }
