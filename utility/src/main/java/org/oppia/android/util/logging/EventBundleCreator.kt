...
class EventBundleCreator
  ...
  constructor(
    ...
    fun fillEventBundle(
      ...
      bundle.putString(
        "oppia_audio_lang",
        eventLog.audioTranslationLanguageSelection.toAnalyticsText(),
      )
      bundle.putInt(
        "screen_density",
        screenDensity,
     ...
     */
    fun fillPerformanceMetricsEventBundle(
      oppiaMetricLog: OppiaMetricLog,
      bundle: Bundle,
    ): String {
      bundle.putInt("android_sdk", androidSdkVersion)
      bundle.putString("app_version_name", appVersionName)
      ...
      return oppiaMetricLog.loggableMetric
        .convertToLoggableMetricType()
        ?.also { loggableMetric ->
    ...
    private class PropertyStore(
      ...
      private val allowUserIds: Boolean,
    ) {
       ...
       */
      fun <T> putSensitiveValue(
        valueName: String,
        value: T,
      ) = putValue(valueName, value, isSensitive = true)

      private fun <T> putValue(
        valueName: String,
        value: T,
        isSensitive: Boolean,
      ) {
      ...
      private fun abbreviateNamespace(namespace: String): String = namespace.split('_').map(String::first).joinToString(separator = "")
    }
     ...
     * [EventLog.Context.getActivityContextCase]).
     */
    private sealed class EventActivityContext<T>(
      ...
      class SubmitAnswerContext(
        ...
        override fun SubmitAnswerEventContext.storeValue(store: PropertyStore) {
          ...
          val adjustedAnswer =
            if (stringifiedAnswer.length > 100) {
      ...
      class EmptyContext(
        activityName: String,
      ) : EventActivityContext<Unit>(activityName, Unit) {
    ...
    private companion object {
      private fun EventLog.Priority.toAnalyticsName() =
        when (this) {
      ...
      private fun OppiaMetricLog.Priority.toAnalyticsName() =
        when (this) {
      ...
      private fun OppiaMetricLog.MemoryTier.toAnalyticsName() =
        when (this) {
      ...
      private fun OppiaMetricLog.StorageTier.toAnalyticsName() =
        when (this) {
      ...
      private fun OppiaMetricLog.NetworkType.toAnalyticsName() =
        when (this) {
      ...
      private fun ScreenName.toAnalyticsName() =
        when (this) {
      ...
      private fun OppiaLanguage.toAnalyticsName() =
        when (this) {
