...
class AndroidLocaleFactory
  ...
  constructor(
    ...
    sealed class LocaleProfileProposal {
      ...
      data class SystemProposal(
        ...
        override fun isViable(localeProfileRepository: LocaleProfileRepository): Boolean =
          localeProfileRepository.availableLocaleProfiles.any { it.matches(profile) } &&
      ...
      data class ForcedProposal(
        ...
        override fun isViable(localeProfileRepository: LocaleProfileRepository): Boolean = minAndroidSdkVersion <= Build.VERSION.SDK_INT
      }
    }
    ...
    class LocaleSource private constructor(
       ...
       */
      fun computeForcedAndroidProposal(): LocaleProfileProposal? = createAndroidResourcesProfile()?.toForcedProposal()
      ...
      private fun toForcedProposal(fallBackToRootProfile: Boolean): LocaleProfileProposal =
        when (val languageTypeCase = languageId.languageTypeCase) {
      ...
      private fun createMacaronicProfile(): AndroidLocaleProfile? = androidLocaleProfileFactory.createFromMacaronicLanguage(languageId)
     ...
     * system locales.
     */
    class MatchedLocalePreferredChooser
      @Inject
      constructor(
        ...
        override fun findBestProposal(
          ...
          primarySource.computeSystemMatchingProposals().findFirstViable(localeProfileRepository)
     ...
     */
    class AndroidResourceCompatibilityPreferredChooser
      @Inject
      constructor(
        private val localeProfileRepository: LocaleProfileRepository,
      ) : ProposalChooser {
    ...
    private companion object {
      private fun List<LocaleProfileProposal>.findFirstViable(repository: LocaleProfileRepository) = firstOrNull { it.isViable(repository) }

      private fun LocaleProfileProposal.takeOnlyIfViable(repository: LocaleProfileRepository) = takeIf { isViable(repository) }
    }
  }
