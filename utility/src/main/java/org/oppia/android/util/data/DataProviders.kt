...
class DataProviders
  ...
  constructor(
    ...
    companion object {
       ...
       */
      fun <I, O> DataProvider<I>.transform(
        newId: Any,
        function: (I) -> O,
      ): DataProvider<O> {
      ...
      fun <I, O> DataProvider<I>.transformNested(
        ...
        NestedTransformedDataProvider.createNestedTransformedDataProvider(
          application,
          newId,
          this,
          function,
          getDataProviders().asyncDataSubscriptionManager,
        )
      ...
      fun <O, T1, T2> DataProvider<T1>.combineWithAsync(
        ...
        return object : DataProvider<O>(application) {
          ...
          override suspend fun retrieveData(): AsyncResult<O> =
            this@combineWithAsync.retrieveData().combineWithAsync(
     ...
     */
    fun <T> createInMemoryDataProvider(
      id: Any,
      loadFromMemory: () -> T,
    ): DataProvider<T> =
      object : DataProvider<T>(application) {
    ...
    fun <T> createInMemoryDataProviderAsync(
      ...
      object : DataProvider<T>(application) {
        ...
        override suspend fun retrieveData(): AsyncResult<T> = loadFromMemoryAsync()
      }
     ...
     */
    fun <T> StateFlow<T>.convertToSimpleDataProvider(id: Any): DataProvider<T> = createInMemoryDataProvider(id) { value }
    ...
    fun <T> StateFlow<T>.convertToAutomaticDataProvider(id: Any): DataProvider<T> =
      convertToSimpleDataProvider(id).also {
        onEach {
    ...
    fun <T> StateFlow<AsyncResult<T>>.convertAsyncToAutomaticDataProvider(id: Any): DataProvider<T> =
      convertAsyncToSimpleDataProvider(id).also {
        onEach {
          // Synchronously notify subscribers whenever the flow's state changes.
          asyncDataSubscriptionManager.notifyChange(id)
        }.launchIn(CoroutineScope(backgroundDispatcher))
    ...
    class NestedTransformedDataProvider<O> private constructor(
      ...
      override suspend fun retrieveData(): AsyncResult<O> = retrieveTransformedData()
