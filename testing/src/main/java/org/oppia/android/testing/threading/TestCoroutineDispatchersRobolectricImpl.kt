 ...
 */
class TestCoroutineDispatchersRobolectricImpl
  @Inject
  constructor(
    ...
    override fun advanceUntilIdle() {
      ...
      while (hasPendingTasks()) {
        val currentTimeMillis = fakeSystemClock.getTimeMillis()
        val taskDelayMillis =
          checkNotNull(advanceToNextFutureTask(currentTimeMillis)) {
    ...
    private fun hasPendingTasks(): Boolean =
      backgroundTestDispatcher.hasPendingTasks() ||
    ...
    private fun getNextUiThreadFutureTaskTimeMillis(timeMillis: Long): Long? =
      uiTaskCoordinator.getNextUiThreadFutureTaskTimeMillis(timeMillis)
    ...
    private class RobolectricUiTaskCoordinator {
      ...
      fun isIdle(): Boolean = isIdleMethod.invoke(shadowUiLooper) as Boolean
      ...
      private fun loadMainShadowLooper(): Any? = shadowLooperClass.getDeclaredMethod("shadowMainLooper").invoke(/* obj = */ null)

      private fun loadIsIdleMethod(): Method = shadowLooperClass.getDeclaredMethod("isIdle")

      private fun loadIdleMethod(): Method = shadowLooperClass.getDeclaredMethod("idle")

      private fun loadGetNextScheduledTaskTimeMethod(): Method = shadowLooperClass.getDeclaredMethod("getNextScheduledTaskTime")
    }
  }
