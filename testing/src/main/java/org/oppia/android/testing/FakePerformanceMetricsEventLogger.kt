...
@Singleton
class FakePerformanceMetricsEventLogger
  @Inject
  constructor() : PerformanceMetricsEventLogger {
    ...
    fun getMostRecentPerformanceMetricsEvent(): OppiaMetricLog = getMostRecentPerformanceMetricsEvents(count = 1).first()

    /** Returns the most recent [count] logged events. */
    fun getMostRecentPerformanceMetricsEvents(count: Int): List<OppiaMetricLog> = performanceMetricsEventList.takeLast(count)
    ...
    fun hasPerformanceMetricsEventLogged(oppiaMetricLog: OppiaMetricLog): Boolean = performanceMetricsEventList.contains(oppiaMetricLog)
