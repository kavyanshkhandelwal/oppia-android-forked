...
class OppiaTestRule : TestRule {
  ...
  private companion object {
    private fun List<TestPlatform>.toPluralPlatformDescription(): String =
      if (size > 1) "platforms ${this.joinToString()}" else "platform ${this.first()}"
    ...
    private fun Description.getTargetTestPlatforms(): List<TestPlatform>? = getAnnotation(RunOn::class.java)?.testPlatforms?.toList()

    private fun <T> Class<T>.getTargetTestPlatforms(): List<TestPlatform>? = getAnnotation(RunOn::class.java)?.testPlatforms?.toList()

    private fun List<BuildEnvironment>.toPluralEnvironmentDescription(): String =
      if (size > 1) "environments ${this.joinToString()}" else "environment ${this.first()}"
    ...
    private fun Description.getTargetBuildEnvironments(): List<BuildEnvironment>? =
      getAnnotation(RunOn::class.java)?.buildEnvironments?.toList()
    ...
    private fun Description.areAccessibilityTestsEnabledForMethod(): Boolean = getAnnotation(DisableAccessibilityChecks::class.java) == null

    private fun <T> Class<T>.areAccessibilityTestsEnabledForClass(): Boolean = getAnnotation(DisableAccessibilityChecks::class.java) == null
  }
}
