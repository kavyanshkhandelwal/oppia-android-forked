...
class DataProviderTestMonitor<T> private constructor(
  ...
  private fun ensureNextResultIsPresent(): AsyncResult<T> {
    ...
    return createCaptor<AsyncResult<T>>()
      .also { resultCaptor ->
    ...
    when (val result = operation()) {
      ...
      is AsyncResult.Failure -> {
        throw IllegalStateException(
          "Expected next result to be a success, not: $result",
          result.error,
        )
      }
  ...
  class Factory
    ...
    constructor(
      ...
      fun <T> createMonitor(dataProvider: DataProvider<T>): DataProviderTestMonitor<T> =
        DataProviderTestMonitor(testCoroutineDispatchers, dataProvider).also {
  ...
  private companion object {
    ...
    private inline fun <reified T> createCaptor(): ArgumentCaptor<T> = ArgumentCaptor.forClass(T::class.java)
